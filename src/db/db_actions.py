...
def process_country_data(dbsess: Session, country: str, country_data: List[str]) -> CountryData:
...
def process_raw_data(country_data: List[str]) -> Dict[str, int]:
    _raw_data = list(map(lambda x: x.replace(',', '').replace('+', '').strip(), country_data))
    _raw_data = [float(res) if (len(res) > 0 and res != 'N/A') else 0 for res in _raw_data]
    columns = ['total_cases', 'new_cases', 'total_deaths', 'new_deaths', 'active_cases',
               'total_recovered', 'serious_critical', 'date_updated']
